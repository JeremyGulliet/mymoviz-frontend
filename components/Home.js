...
function Home() {
  ...
  useEffect(() => {
    fetch('https://mymoviz-backend-three-theta.vercel.app/movies')
  ...
  const movies = moviesData.map((data, i) => {
    ...
    const overview = `${data.overview.substr(0, 249)}...`;
    const isLiked = likedMovies.some(movie => movie === data.title);
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={overview} poster={poster} voteAverage={data.vote_average} voteCount={data.vote_count} />;
    });
